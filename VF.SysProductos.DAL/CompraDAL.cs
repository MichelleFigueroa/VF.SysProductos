...
namespace VF.SysProductos.DAL
{
    public class CompraDAL
    {
        ...
        public CompraDAL(VFSyProductosDBContext sysProductosDB)
        {
        ...
        public async Task<int> AnularAsync(int idCompra)
        {
            ...
            if (compra == null || compra.Estado == (byte)EnumEstadoCompra.Anulada)
            {
                return 0; // Si ya estaba anulada, no hacer nada
            }

            // Marcar la compra como anulada
            compra.Estado = (byte)EnumEstadoCompra.Anulada;
